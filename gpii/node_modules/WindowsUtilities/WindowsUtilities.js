...
windows.user32 = ffi.Library("user32", {
    ...
    // https://msdn.microsoft.com/library/windows/desktop/ms645505
    "MessageBoxW":  [
        "int", [t.HANDLE, "char*", "char*", t.UINT]
    ]
});
...
windows.API_constants = {
    ...
    // https://msdn.microsoft.com/library/ms645505.aspx
 ...
 * This function will block until the user dismisses the message.
 *
 * @param text {String} The main message.
 * @param caption {String} The window title.
 * @param icon {String} The icon, one of: warn, info, or error.
 */
windows.showMessage = function (text, caption, icon) {
    var type = 0;
    switch (icon) {
    case "warn":
        type = c.MB_ICONWARNING;
        break;
    case "info":
        type = c.MB_ICONINFORMATION;
        break;
    case "error":
        type = c.MB_ICONERROR;
        break;
    ...
    fluid.log("Showing message: [" + caption + "] " + text);

    var textBuf = windows.stringToWideChar(text);
    var captionBuf = windows.stringToWideChar(caption);
    windows.user32.MessageBoxW(0, textBuf, captionBuf, type);
};
