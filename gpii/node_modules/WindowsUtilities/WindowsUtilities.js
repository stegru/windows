...
gpii.windows.getUsbDrives = function (drivePaths) {
    ...
    drives = drives.filter(function (drivePath) {
        ...
        return busType === BusTypeUsb && gpii.windows.checkMountedDrive(drivePath);
    });
...
/**
 * Determines if a drive has been mounted.
 * @param {String} drivePath A path on the drive to check.
 * @return {Boolean} true if the drive is mounted.
 */
gpii.windows.checkMountedDrive = function (drivePath) {
    // If the root directory exists, then it is mounted.
    var parsed = path.parse(path.resolve(drivePath));
