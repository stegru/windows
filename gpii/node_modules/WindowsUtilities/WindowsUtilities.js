...
windows.kernel32 = ffi.Library("kernel32", {
    ...
    "GetApplicationRestartSettings": [
        ...
        t.HANDLE, [ t.HANDLE, t.HANDLE, t.LPDWORD, t.HANDLE]
    ]
});
...
windows.FILETIME = new Struct([
    [t.DWORD, "dwLowDateTime"],
    [t.DWORD, "dwHighDateTime"]
]);

// https://msdn.microsoft.com/library/aa373677
windows.RM_UNIQUE_PROCESS = new Struct([
    [t.DWORD, "dwProcessId"],
    [windows.FILETIME, "ProcessStartTime"]
]);
windows.RM_UNIQUE_PROCESSES = arrayType(windows.RM_UNIQUE_PROCESS);

// Restart Manager API: https://msdn.microsoft.com/library/aa373524
windows.restartManager = ffi.Library("rstrtmgr", {
    // https://msdn.microsoft.com/library/aa373668
    "RmStartSession": [
        t.DWORD, [ t.LPDWORD, t.DWORD, "char*" ]
    ],
    // https://msdn.microsoft.com/library/aa373663
    "RmRegisterResources": [
        t.DWORD, [ t.DWORD, t.UINT, "char*", t.UINT, windows.RM_UNIQUE_PROCESSES, t.UINT, "void*" ]
    ],
    // https://msdn.microsoft.com/library/aa373665
    "RmRestart": [
        t.DWORD, [ t.DWORD, t.DWORD, "void*" ]
    ],
    // https://msdn.microsoft.com/library/aa373667
    "RmShutdown": [
        t.DWORD, [ t.DWORD, t.ULONG, "void*" ]
    ],
    // https://msdn.microsoft.com/library/aa373659
    "RmEndSession": [
        t.DWORD, [ t.DWORD ]
    ]
});
...
windows.createStructArray = function (itemType, itemCount) {
    return new (arrayType(itemType))(itemCount);
};
 ...
 */
windows.getWindowProcess = function (hwnd) {
    var pid = null;
    if (hwnd) {
...
/**
 * Posts a message to one or more windows (doesn't wait for the response).
 *
 * See PostMessage (https://msdn.microsoft.com/library/ms644944).
 *
 * @param hwnd {Number|Number[]} The recipient window handle(s).
 * @param msg {Number} The message to post.
 * @param wParam {Number} Message specific data.
 * @param lParam {Number} Message specific data.
 */
windows.postMessage = function (hwnd, msg, wParam, lParam) {
    fluid.each(fluid.makeArray(hwnd), function (w) {
        windows.user32.PostMessageW(w, msg, wParam, lParam);
    });
};
