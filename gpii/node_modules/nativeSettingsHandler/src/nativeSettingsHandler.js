 ...
 * @param {Number} [op] The operation to be performed on the system. Possible values are "Volume" or "Mute".
 * @param {Number} [num] The value to set as the current system volume. Range is normalized from 0 to 1.
 * @return {Promise} A promise that resolves on success or holds a object with the error information.
 */
windows.nativeSettingsHandler.VolumeHandler = function (mode, op, num) {
    ...
    try {
        ...
        if (mode === "Get") {
            valueBuff = child_process.execFileSync(fileName, [mode, op], {});
        } else {
            valueBuff = child_process.execFileSync(fileName, [mode, op, num], {});
        }
...
windows.nativeSettingsHandler.GetVolume = function () {
    return windows.nativeSettingsHandler.VolumeHandler("Get", "Volume");
};
...
windows.nativeSettingsHandler.SetVolume = function (num) {
    return windows.nativeSettingsHandler.VolumeHandler("SetNear", "Volume", num).then(function (result) {
...
/**
 * Gets the current system audio channel mute state, calling the native executable 'VolumeControl.exe' with
 * the proper payload.
 *
 * @return {Promise} A promise that resolves on success or holds a object with the error information.
 */
windows.nativeSettingsHandler.GetMute = function () {
    return windows.nativeSettingsHandler.VolumeHandler("Get", "Mute");
};

/**
 * Sets the current system audio channel mute state, calling the native executable 'VolumeControl.exe' with
 * the proper payload.
 *
 * @param {Number} num The value to set as the current system audio channel mute state. Value can be '0' or '1'.
 * @return {Promise} A promise that resolves on success or holds a object with the error information.
 */
windows.nativeSettingsHandler.SetMute = function (num) {
    return windows.nativeSettingsHandler.GetMute().then(function (muted) {
        var promise = fluid.promise();
        if (muted == num) {
            promise.resolve();
        } else {
            gpii.windows.messages.sendMessage("Shell_TrayWnd", windows.API_constants.WM_APPCOMMAND, 0,
                windows.makeLong(0, windows.API_constants.APPCOMMAND_VOLUME_MUTE));
            promise.resolve(num);
        }
        return promise;
    });
...
windows.nativeSettingsHandler.functions = {
    ...
    Mute: {
        set: windows.nativeSettingsHandler.SetMute,
        get: windows.nativeSettingsHandler.GetMute
    }
};
