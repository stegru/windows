...
gpii.tests.windows.nativeSettingsHandler.muteApiTest = {
    init: {
        "settings": {
            "Mute": {
                "value": 0
...
jqUnit.test("Testing nativeSettingsHandler mute API (get and set functions)", function () {
    jqUnit.expect(2);

    // Set to initial values:
    gpii.windows.nativeSettingsHandler.setImpl(gpii.tests.windows.nativeSettingsHandler.muteApiTest.init);
    // First we test the get functionality
    var result = gpii.windows.nativeSettingsHandler.get(gpii.tests.windows.nativeSettingsHandler.muteApiTest.get.payload);
    // Check results
    jqUnit.assertDeepEq("Checking return payload of get", gpii.tests.windows.nativeSettingsHandler.muteApiTest.get.expectedResult, gpii.resolveSync(result));

    // Test set functionality
    var result2 = gpii.windows.nativeSettingsHandler.set(gpii.tests.windows.nativeSettingsHandler.muteApiTest.set.payload);
    // Check results
    jqUnit.assertDeepEq("Checking return payload of set", gpii.tests.windows.nativeSettingsHandler.muteApiTest.set.expectedResult, gpii.resolveSync(result2));

    // Unmute again
    gpii.windows.nativeSettingsHandler.setImpl(gpii.tests.windows.nativeSettingsHandler.muteApiTest.final);
});
