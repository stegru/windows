...
gpii.windows.closeAppWithMinForce = function (processToClose, options) {
    ...
    var tryCloseWithHighForce = function () {
        gpii.windows.closeApplications("high", {pids: procPids, restart: options.restart, quitDelay: options.quitDelay})
            .then(
                function () {
                    gpii.windows.waitForProcessTermination(processToClose, { timeout: options.timeout })
                        .then(
                            function () {
                                promise.resolve();
                            },
...
/**
 * Close a running application after a command has been executed.
 *
 ...
 * @param processToWait {String} The process that is going to be closed/oppened after the command execution.
 * @param processToClose {String} The process for closing after the processToWait has exited.
 * @param options {Object} Optional.
 * @param options.commandType {String} Specificies if command is supposed to "Open" or "Close" the application.
...
gpii.windows.closeAfterCommand = function (command, args, processToWait, processToClose, options) {
    ...
    // Check if the 'commandType' that is going to be executed is valid.
    var validCommands = ["Open", "Close"];

    if (validCommands.indexOf(options.commandType) === -1) {
        promise.reject({
            isError: true,
            message: "gpii.windows.closeAfterCommand: Error invalid commandType option '" + options.commandType + "'."
        });
        return promise;
    }
    ...
    var keepTrying = function (rPromise, retries) {
        if (retries <= 0) {
            if (options.commandType === "Close") {
                fluid.log("gpii.windows.closeAfterCommand: Timeout for process closing failed after '" + retries + "' retries.");
                rPromise.reject({
                    isError: true,
                    message: "Exceeded the number of retries trying to close process '" + processToClose + "'."
                });
            } else if (options.commandType === "Open") {
                fluid.log("gpii.windows.closeAfterCommand: Timeout for process openning failed after '" + retries + "' retries.");
                rPromise.reject({
                    isError: true,
                    message: "Exceeded the number of retries trying to open process '" + processToClose + "'."
                });
            }
        } else {
                    ...
                    process, options)
                        .then(
                            ...
                            function () {
                                p.reject({
                                    isError: true,
                                    message: "gpii.windows.closeAfterCommand: Unable to close application '" + process
                                });
                            }
                        );
            };
            ...
            if (options.commandType === "Open") {
                gpii.windows.waitForProcessStart(
                    processToWait, {timeout: options.commandTimeout, pollDelay: options.pollDelay})
                        .then(
                            function () {
