...
gpii.windows.restartProcesses = function (pids, options) {
    ...
    // HACK: For ATIA Demo - make sure explorer restarts
    // Wait for explorer to die. If it doesn't restart after 5 seconds, then start it manually.
    if (!options.shutdownOnly) {
        var child_process = require("child_process");
        fluid.log("Waiting for explorer's death");
        windows.waitForProcessTermination("explorer.exe", {timeout: 20000}).then(function () {
            fluid.log("explorer died - waiting for restart");
            windows.waitForProcessStart("explorer.exe", {timeout: 5000}).then(function () {
                fluid.log("explorer restarted");
            }, function () {
                fluid.log("explorer didn't start. starting manually");
                child_process.exec("c:\\windows\\explorer.exe");
            });
        }, function () {
            fluid.log("explorer didn't die (or was restarted fast)");
        });
    }
