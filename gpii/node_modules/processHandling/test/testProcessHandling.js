...
jqUnit.asyncTest("Testing closeAfterCommand", function () {
    jqUnit.expect(1);

    gpii.tests.windows.processHandling.startTestProcess().then(function () {
        gpii.windows.closeAfterCommand(
            "cmd.exe", ["dir"], "test-window.exe", "test-window.exe", {commandType: "Open", timeout: 1000, commandTimeout: 1000})
                .then(
                    function () {
                        jqUnit.assert("closeAfterCommand promise resolved.");
                        jqUnit.start();
                    },
                    function (err) {
                        jqUnit.fail(err);
...
gpii.tests.windows.mockCloseApplicationLevels = [];
...
gpii.tests.windows.processHandling.mockCloseApplications = function () {
    ...
    gpii.windows.closeApplications = function (level) {
        ...
        var preLevels = gpii.tests.windows.mockCloseApplicationLevels;
        ...
        } else if (preLevels.length === 1) {
            jqUnit.assertDeepEq("test: Previous calls should be done in order",
                preLevels, [1]);
            jqUnit.assertEquals("test: Second call should be done with level 2",
                levelNum, 2);
        ...
        } else if (preLevels.length === 2) {
            jqUnit.assertDeepEq("test: Previous calls should be done in order",
                preLevels, [1,2]);
            jqUnit.assertEquals("test: Second call should be done with level 3",
                levelNum, 3);
...
jqUnit.asyncTest("Testing closeAppWithMinForce", function () {
    ...
    gpii.windows.closeAppWithMinForce("explorer.exe", {restart: true, quitDelay: 1});
});
