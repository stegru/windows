 ...
 * @return {Promise} Resolves when the AT has been started or stopped.
 */
gpii.windows.enableRegisteredAT = function (name, enable, options) {
    ...
    if (!options.configOnly) {
        if (enable) {
            processPromise = gpii.windows.startRegisteredAT(name, options);
        } else {
            processPromise = gpii.windows.stopRegisteredAT(name, options);
        }
    } else {
        processPromise = fluid.promise();
    ...
    if (enable && !options.configOnly) {
        fluid.promise.follow(processPromise, promiseTogo);
    } else {
        ...
        fluid.promise.sequence([processPromise, gpii.windows.whilePendingAT]).then(function () {
 ...
 * @return {Promise} A promise that will resolve when the application has started.
 */
gpii.windows.startRegisteredAT = function (name, options) {
    ...
    gpii.windows.waitForProcessTermination("utilman.exe").then(function () {
        gpii.windows.nativeExec(command, {}, function () {
            var p = gpii.windows.waitForProcessStart(atInfo.ATExe);
...
gpii.windows.stopRegisteredAT = function (name, options) {
    ...
    return gpii.windows.waitForProcessTermination(exeName);
};
