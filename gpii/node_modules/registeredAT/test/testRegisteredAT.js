...
jqUnit.asyncTest("Testing AT start and stop", function () {
    ...
    var promise = gpii.windows.startRegisteredAT(testData.appName, {atInfo: testData});
    jqUnit.assertTrue("startRegisteredAT should return a promise", fluid.isPromise(promise));

    // Wait for it to start.
    promise.then(function () {
        var pid = gpii.windows.findProcessByName(testData.ATExe);
        jqUnit.assertTrue("The process should be running.", !!pid);

        // Tell the process to stop now.
        var stopPromise = gpii.windows.stopRegisteredAT(testData.appName, {atInfo: testData});
        jqUnit.assertTrue("stopRegisteredAT should return a promise", fluid.isPromise(stopPromise));

        // Wait for it to die
        stopPromise.then(function () {
            var pid = gpii.windows.findProcessByName(testData.ATExe);
            jqUnit.assertTrue("The process should no longer be running.", !pid);
            jqUnit.start();
        }, function () {
